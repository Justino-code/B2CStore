...
<div {{ $attributes->merge(['class' => "rounded-md p-4 border {$config['bg']} {$config['border']}"]) }}>
    <div class="flex">
        <div class="flex-shrink-0">
            <svg class="h-5 w-5 {{ $config['text'] }}" viewBox="0 0 20 20" fill="currentColor">
        ...
        @if($dismissible)
            <div class="ml-auto pl-3">
                <div class="-mx-1.5 -my-1.5">
                    <button type="button" class="inline-flex rounded-md p-1.5 {{ $config['text'] }} hover:bg-{{ $type }}-100 dark:hover:bg-{{ $type }}-800">
